...
def is_prefix(s: str, pattern: str):
...
def can_build_string(
    ...
    for pattern in patterns:
        if is_prefix(s, pattern) and can_build_string(s[len(pattern) :], patterns):
            return True
    return False


@functools.cache
def num_ways_to_build_string(
    s: str,
    patterns: frozenset[str],
) -> int:
    if s == "":
    ...
    total = 0
    for pattern in patterns:
        if is_prefix(s, pattern):
            total += num_ways_to_build_string(s[len(pattern) :], patterns)
    return total


def test_can_build_string():
    patterns = frozenset(
        [
            "r",
            "wr",
    ...
    assert can_build_string("brwrr", patterns) is True
    assert can_build_string("bggr", patterns) is True
    assert can_build_string("gbbr", patterns) is True
    assert can_build_string("rrbgbr", patterns) is True
    assert can_build_string("ubwu", patterns) is False
    assert can_build_string("bwurrg", patterns) is True
    assert can_build_string("brgr", patterns) is True
    assert can_build_string("bbrgwb", patterns) is False


def test_num_ways_to_build_string():
    patterns = frozenset(
        [
            "r",
            "wr",
    ...
    assert num_ways_to_build_string("brwrr", patterns) == 2
    # assert num_ways_to_build_string("bggr", patterns) is True
    # assert num_ways_to_build_string("gbbr", patterns) is True
    # assert num_ways_to_build_string("rrbgbr", patterns) is True
    # assert num_ways_to_build_string("ubwu", patterns) is False
    # assert num_ways_to_build_string("bwurrg", patterns) is True
    # assert num_ways_to_build_string("brgr", patterns) is True
    # assert num_ways_to_build_string("bbrgwb", patterns) is False
...
if __name__ == "__main__":
    ...
    for string in STRINGS:
        if can_build_string(string, PATTERNS):
            count += 1

    total_num_ways = 0
    for string in STRINGS:
        total_num_ways += num_ways_to_build_string(string, PATTERNS)
